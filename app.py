...
#This program is used to gather the dataset
...
def save_resized_image(image, folder_path, label, brect):
    ...
    file_name = os.path.join(folder_path, f"{label}.jpg")
    # Save resized ROI
...
def main():
    ...
    while True:
        ...
        for x in range(0, len(options)):
            if key == ord(options[x]):
                label = options[x] + str(counts[options[x]])
                folder_path = f"{options[x].upper()}/"
                if not os.path.exists(folder_path):
                    os.makedirs(folder_path)

                # Save resized image
                save_resized_image(debug_image, folder_path, label, brect)


                data = {"label": label, "skeleton_data": landmark_list}  # Example data to append
